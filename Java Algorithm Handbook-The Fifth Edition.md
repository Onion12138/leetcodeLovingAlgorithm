...
|[1334. 阈值距离内邻居最少的城市](https://leetcode.cn/problems/find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance/) | 中等 | Floyed算法
...
其状态转移方程定义为$dp[i][j] = \max(dp[i-1][j], dp[i-1][j-w[i]]+v[i])$
...
和零一背包对比，完全背包的状态转移仅仅在于i和i-1的区别。

空间压缩优化：

完全背包当前状态依赖于左侧和上方的状态，采用从上到下，从左到右的顺序更新。

零一背包当前状态依赖于左上方和上方的状态，采用从上到下，从右到左的顺序更新。
