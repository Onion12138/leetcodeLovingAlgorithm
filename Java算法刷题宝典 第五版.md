...
class Solution {
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        for(int i = 0; i < nums.length; i ++) {
            if(i >= k && nums[i - k] == nums[queue.peekFirst()]) {
                queue.pollFirst();
            }
...
3. 采用双向广度优先遍历会超时。虽然想把这道题作为双向BFS的例题，😓。
4. 后续可能还有针对性的测试用例将本方法卡超时，需要考虑近一步剪枝。
...
class Solution {
    public int minCostConnectPoints(int[][] points) {
        ...
        while(!queue.isEmpty()) {
            ...
            ans += w;
            int newV = visited[from] ? to : from;
            visited[newV] = true;
            for(int i = 0; i < n; i ++) {
                if(i != newV && !visited[i]) {
                    queue.offer(new int[]{newV, i, getDistance(points, newV, i)});
                }
            }
        }
        return ans;
...
| 算法            | 时间复杂度                  | 特点                             |
| --------------- | --------------------------- | -------------------------------- |
| Dijkstra算法    | $O((V+E)\log V)$ 基于二叉堆 | 不能含负权边，单源最短路径       |
| BellmanFord算法 | $O(V*E)$                    | 可以检测负权环，单源最短路径     |
| SPFA算法        | $O(k*E)$，$k$为平均入队次数 | 可以检测负权环，单源最短路径     |
| Floyed算法      | $O(V^3)$                    | 可以检测负权环，所有点对最短路径 |
